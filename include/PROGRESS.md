...
- Measured FPS: 27.2 steady (render 36–38 ms; compose ~32 ∥ push ~34)
...
- PERF-3 (open, minor): faint band tearing — band writes not synced to panel scanout. Cosmetic at current severity; revisit only if real art makes it visible.
...
- 2026-07-12: render interpolation (prev/curr + alpha) added for parallax; fixes 60 Hz-update vs ~27 fps-render scroll jerk.
...
- fps ledger, every step measured on hardware: 11.4 (baseline PSRAM layers) → 13.8 (pclk 5 MHz) → 17.5 (flash layers + row compositor) → 21.7 (row-skip flags) → 25.4 (dual-core pipeline) → 27.2 (pclk 4 MHz).
...
- v0.2.0 — 2026-07-12 — M1: render skeleton on hardware. 27.2 fps, 3 parallax layers + 10 sprites, dual-core pipeline. Target revised to 25 fps floor.
